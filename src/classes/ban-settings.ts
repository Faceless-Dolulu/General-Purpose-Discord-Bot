...
export class BanSettings extends ModerationSettings {
	constructor(
		guildId: Snowflake,
		enabled: boolean,
	...
	protected override hasChanges(config: BanSettings): boolean {
		if (
	...
	override async saveToDatabase(): Promise<UpdateWriteOpResult> {
		return await banCommandSettings.updateOne(
			{
	...
	override async createSettingsContainer(
		...
		if (properties.saved === true) {
			title = `## Ban Command Configuration (Changes Saved)` as string;
		} else if (this.hasChanges(config) === true) {
			title = `## Ban Command Configuration (Changes Unsaved)` as string;
		} else {
			title = `## Ban Command Configuration` as string;
		}
