...
export class WarnSettings extends ModerationSettings {
	constructor(
		guildId: Snowflake,
		enabled: boolean,
	...
	protected override hasChanges(config: WarnSettings): boolean {
		if (
	...
	override async saveToDatabase(): Promise<UpdateWriteOpResult> {
		return await warnCommandSettings.updateOne(
			{
	...
	override async createSettingsContainer(
		...
		if (properties.saved === true) {
			title = `## Warn Command Configuration (Changes Saved)` as string;
		} else if (this.hasChanges(config) === true) {
			title = `## Warn Command Configuration (Changes Unsaved)` as string;
		} else {
			title = `## Warn Command Configuration` as string;
		}
		...
		container.addActionRowComponents(
			new ActionRowBuilder<ChannelSelectMenuBuilder>().addComponents(
				createSelectMenu({
					...
					placeholder: `Select a log channel for warn actions`,
